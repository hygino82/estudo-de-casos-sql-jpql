...
public class Uri2737Application implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		List<LawyerMinProjection> list = repository.buscarAdvogadosComMaximoClientesMininoMedia();
		List<LawyerMinDTO> res = list.stream().map(x -> new LawyerMinDTO(x)).collect(Collectors.toList());
